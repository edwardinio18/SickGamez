<?php
	...
	if (isset($_GET['followUnfollow']) || isset($_GET['deleteFeedPost']) || isset($_GET['submitComment'])) {
		...
		$query = "SELECT * FROM feed INNER JOIN users ON feed.feedUserId = users.id ORDER BY dateAndTimeFeedSubmitted DESC LIMIT " . $start . ", " . $limit . ";";
		...
		if ($result->num_rows >= 1) {
			...
			while ($row = $result->fetch_assoc()) {
				...
				$feedImagePath = "/SickGamez/Feed_images/" . $feedImage;
