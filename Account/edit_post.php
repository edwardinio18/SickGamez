...
<head>
	...
	<style type="text/css">
	    ...
	    #post_not_accessible_not_logged_in {
	    ...
	    .uploadNewPostImageLabel:active {
	    ...
	    #deletePostImage:active {

	    	background-color: white;

	    }

	    #no_current_post_image {
...
<?php
	...
	if (isset($_GET['id'])) {
		...
		$query = "SELECT post FROM posts WHERE postId = ?;";
		...
		if ($stmt->num_rows > 0) {
			...
			if ($stmt->fetch()) {
				...
				$postContent = $post;
				$decryptedCurrentPost = decrypt($postContent, $key);
	...
	if (isset($_POST['edit_button'])) {
		...
		$postsTrim = trim($postContent);
		$strippedTagsPost = strip_tags($postsTrim);
		$encryptedPost = encrypt($strippedTagsPost, $key);

		$newUploadedPostImageName = $_FILES['uploadNewPostImage']['name'];
		$newUploadedPostImageSize = $_FILES['uploadNewPostImage']['size'];
		$newUploadedPostImageSource = $_FILES['uploadNewPostImage']['tmp_name'];
		$newUploadedPostImageType = $_FILES['uploadNewPostImage']['type'];
		$newUploadedPostImageTargetPath = "/home/yv4nbnligki0/public_html/Profile_posts_images/";

		if (empty($newUploadedPostImageName)) {

			if ($strippedTagsPost === " " || (empty($strippedTagsPost))) {

				$strippedTagsPost = "";

				$updatePostQuery = "UPDATE posts SET post = ?, dateAndTimeAdded = ? WHERE postId = ?;";
				$stmt = mysqli_stmt_init($db);

				if (!mysqli_stmt_prepare($stmt, $updatePostQuery)) {
					...
					$newTimeAndDateAdded = date("Y-m-d H:i:s");
					$id = $_GET['id'];

					$strippedTagsPost = "";

					mysqli_stmt_bind_param($stmt, "sss", $strippedTagsPost, $newTimeAndDateAdded, $id);
					mysqli_stmt_execute($stmt);

					$username = $_COOKIE['username'];

					echo "<meta http-equiv='refresh' content='0; url=posts?username=$username&post_edited=success' />";
				...
				if (!mysqli_stmt_prepare($stmt, $updatePostQuery)) {
					...
					mysqli_stmt_bind_param($stmt, "sss", $encryptedPost, $newTimeAndDateAdded, $id);
					mysqli_stmt_execute($stmt);
			...
			if ($strippedTagsPost === " " || (empty($strippedTagsPost))) {
				...
				if ($newUploadedPostImageSize > 5242880) {
				...
				if ($errors == 0) {
					...
					$currentPostImageDB = "";
					$currentPostImageDBPath = "/home/yv4nbnligki0/public_html/Profile_posts_images/";
					...
					if ($result->num_rows >= 1) {
						...
						if ($row1 = $result->fetch_assoc()) {
							...
							$currentPostImageDB = $row1['postImage'];

							$query = "UPDATE posts SET post = ?, postImage = ?, dateAndTimeAdded = ? WHERE postId = ?;";
							$stmt = mysqli_stmt_init($db);
							...
							if (!mysqli_stmt_prepare($stmt, $query)) {
								...
								$newUploadedPostImageName = time() . ".png";
								$postId = $_GET['id'];
								$newTimeAndDateAdded = date("Y-m-d H:i:s");

								$strippedTagsPost = "";

								mysqli_stmt_bind_param($stmt, "ssss", $encryptedPost, $newUploadedPostImageName, $newTimeAndDateAdded, $postId);
								mysqli_stmt_execute($stmt);
								...
								if ($row1['postImage'] !== "") {
									...
									unlink($currentPostImageDBPath . $currentPostImageDB);

									move_uploaded_file($newUploadedPostImageSource, $newUploadedPostImageTargetPath . $newUploadedPostImageName);
	...
	if (isset($_POST['delete_button'])) {
		...
		if ($result->num_rows >= 1) {
			...
			if ($row = $result->fetch_assoc()) {
				...
				$currentPostImageDB = $row['postImage'];

				if ($currentPostImageDB === "") {

					$deletePostQuery = "DELETE FROM posts WHERE username = ? AND postId = ?;";
					$stmt = mysqli_stmt_init($db);

					if (!mysqli_stmt_prepare($stmt, $deletePostQuery)) {
						...
						echo "<meta http-equiv='refresh' content='0; url=posts?username=$username&post_deleted=success' />";
	...
	if (isset($_POST['delete_post_image_button'])) {
		...
		if ($result->num_rows >= 1) {
			...
			if ($row = $result->fetch_assoc()) {
				...
				if ($currentPostImageDB === "") {
					...
					$noCurrentPostImage = "No image found on this current post!";
					$errors = 1;
					...
					if (empty($strippedTagsPost) || $strippedTagsPost === " ") {
						...
						$postId = $_GET['id'];
						$newTimeAndDateAdded = date("Y-m-d H:i:s");
						$stmt->bind_param("ssss", $encryptedPost, $postImage, $newTimeAndDateAdded, $postId);
						$stmt->execute();
						...
						echo "<meta http-equiv='refresh' content='0; url=posts?username=$username&post_image_deleted=success' />";
...
<h1 id="edit_post_label">Edit post</h1>

<p id="uploaded_image_too_big"><?php echo $uploadedImageError; ?></p>

<p id="no_current_post_image"><?php echo $noCurrentPostImage; ?></p>

	<?php

		$editPostQuery = "SELECT * FROM posts WHERE postId = ?;";
		$stmt = $db->prepare($editPostQuery);
		$postId = $_GET['id'];
		$stmt->bind_param("s", $postId);
		$stmt->execute();
		$editPostQueryResult = $stmt->get_result();

		if ($editPostQueryResult->num_rows >= 1) {

			if ($row = $editPostQueryResult->fetch_assoc()) {
				...
				?>
					...
					<div class="formDiv">
							...
							if ($result->num_rows >= 1) {
								...
								if ($row = $result->fetch_assoc()) {
									...
									if ($postImageDB === "") {
										...
										$postImageDBPath = "/Profile_posts_images/" . $postImageDB;

										?>

											<div id="current_post_image_div">
										
												<img src="<?php echo $postImageDBPath; ?>" id="current_post_image" />
						...
						<div id="current_post_image_main_outer_div"></div>

						<form action="" method="post" enctype="multipart/form-data" style="text-align: center; margin-top: 2%;">

							<textarea name="postcontent" id="postcontent" maxlength="400" placeholder="Add a post description..." onfocus="this.placeholder = 'Add a post description...'" onblur="this.placeholder = 'Add a post description...'"><?php echo $decryptedCurrentPost; ?></textarea>

							<br />
							
							<label for="uploadNewPostImage" class="uploadNewPostImageLabel" id="uploadNewPostImageLabel">Choose an image!</label>

							<input type="file" name="uploadNewPostImage" id="uploadNewPostImage" class="uploadNewPostImage" accept=".png, .jpg, jpeg" />

							<br />

							<input type="submit" id="deletePostImage" name="delete_post_image_button" value="Delete post image!" />

							<br />

							<input type="submit" id="editButton" name="edit_button" value="Edit post!" />

							<br />

							<input type="submit" id="deleteButton" name="delete_button" value="Delete post!" />
...
<?php if ($login == 1) : ?>
	...
	<?php
		...
		$query = "SELECT username FROM posts WHERE postId = ?;";
		$stmt = $db->prepare($query);
		$stmt->bind_param("s", $id);
		$stmt->execute();
		$stmt->store_result();
		...
		if ($stmt->num_rows > 0) {
			...
			if ($stmt->fetch()) {
				...
				if ($currentUsername == $username) {
					...
					$editPostError = "Error";
					$editPostLoggedInDifferentUser = "You can't edit this post!";
					...
					?>
						...
						<h1 id="post_error_label"><?php echo $editPostError; ?></h1>

						<p id="post_logged_in_different_user"><?php echo $editPostLoggedInDifferentUser; ?></p>
			...
			$editPostError = "Error";
			$postNotFound = "No post linked with the provided post id in the URL exists!";
			...
			?>
				...
				<p id="post_not_found"><?php echo $postNotFound; ?></p>
	...
	<div id="back_arrow_div">
		...
		<a href="posts?username=<?php echo $username; ?>" id="back_arrow_link">
...
<?php if ($login == 0) : ?>
	...
	<?php
		...
		$editPostError = "Error";
		$editPostNotLoggedIn = "Edit post can't be accessed if not logged in!";
	...
	<p id="post_not_accessible_not_logged_in"><?php echo $editPostNotLoggedIn; ?></p>
...
<script type="text/javascript">
	...
	$(".uploadNewPostImage").change(function(e) {

		var file = $(".uploadNewPostImage")[0].files[0].name;
		$(this).prev("label").text(file);

		var tmpPath = URL.createObjectURL(e.target.files[0]);

		$("#current_post_image").attr("src", tmpPath);

		if (!($("#current_post_image_div").is(":visible"))) {

			$("#current_post_image_main_outer_div").append('<div id="current_post_image_div"><img src="' + tmpPath + '" id="current_post_image" /></div>');
