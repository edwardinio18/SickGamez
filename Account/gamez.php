<?php
	...
	$profileNotAccessible = "User gamez can't be accessed if not logged in!";
	$profileAndUserError = "Error";
	$userNotFoundLoggedIn = "User doesn't exist!";
	$profileNotAccessibleLoggedIn = "User gamez can't be accessed!";
	$gamesListForOthers = "";
	$gameCoverImage = "";
	$gameEditedSuccessfully = "";
...
<head>
	...
	<style type="text/css">
	    ...
	    #gamez_title_div_for_others:hover #gamez_title_for_others  {
	    ...
	    #edit_gamez_button:active {

	    	background-color: white;

	    }

	    .gamez_for_others_not_logged_in {
	    ...
	    .gamez_for_others_not_logged_in:hover {
	    ...
	    .gamez_for_others_not_logged_in::before {
	    ...
	    #gamez_for_others_not_logged_in_title {
	    ...
	    #gamez_for_others_not_logged_in_title_div {
	    ...
	    #gamez_for_others_not_logged_in_title_div:hover #gamez_for_others_not_logged_in_title  {
	    ...
	    #gamez_for_others_not_logged_in_button {
	    ...
	    #gamez_label_for_others_not_logged_in {
	    ...
	    #number_of_gamez_for_others_not_logged_in {
	    ...
	    #gamez_not_found_for_others_not_logged_in {
					...
					?>
						...
						<script type="text/javascript">
							...
							$(document).ready(function() {
								...
								$("#gamez_label_for_others").remove();
								$("#gamez_label_for_others_2").remove();
								$("#number_of_gamez_for_others").remove();
								$("#gamez_not_found_for_others").remove();
								$(".gamez_for_others").remove();
								$("#gamez_not_found_2").remove();
...
<?php
	...
	if ($login == 1 && $result->num_rows > 0 && $_GET['username'] === $usernameCookie || $login == 0 && $result->num_rows > 0 && $_GET['username'] === $usernameCookie) {
		...
		if ($selectGameQueryResult->num_rows >= 1) {
			...
			$selectGamesQuery = "SELECT username, activeStatus, gameCoverImage, gameName, gameId FROM gamez WHERE username = ? AND activeStatus = ?;";
			...
			while ($row = $selectGamesQueryResult->fetch_assoc()) {
				...
				if ($usernameDB === $currentUsername) {
					...
					if ($gameActiveStatus == 1) {
						...
						?>
							...
							$gamesList .= "<li class='gamez' onclick=\"window.location.href='/game?id=" . $gameID . "'\">
												...
												<div id='gamez_image' class='gamez_image' style='background-image: url(" . $gameCoverImage . ");'>
													
													<button id='gamez_button'></button>
											...
											<form action='edit_game?id=" . $gameID . "' method='post'>
													
												<input type='submit' name='edit_gamez_button' id='edit_gamez_button' value='Edit' />
					...
					$noGames = "Your gamez list is currently empty!";
		...
		if (isset($_GET['username']) && $result->num_rows > 0 && $_GET['username'] == $_COOKIE['username']) {

			if ($atLeastOneGameUploaded == 0) {

				$selectUserGameQuery = "SELECT * FROM gamez WHERE username = ? AND activeStatus = ?;";
				...
				if ($selectUserGameQueryResult->num_rows >= 1) {
					...
					while ($row = $selectUserGameQueryResult->fetch_assoc()) {

						$gameCounter = mysqli_num_rows($selectUserGameQueryResult) . " gamez active";

						if ($gameCounter == 1) {

							$gameCounter = mysqli_num_rows($selectUserGameQueryResult) . " game active";
			...
			$selectNumberOfGamesQuery = "SELECT * FROM gamez;";
			$stmt->prepare($selectNumberOfGamesQuery);
			$stmt->execute();
			$selectNumberOfGamesQueryResult = $stmt->get_result();

			if ($selectNumberOfGamesQueryResult->num_rows == 0) {
			...
			if (isset($_GET['game_edited'])) {

				if ($_GET['game_edited'] == "success") {

					$gameEditedSuccessfully = "Your game has been edited and submited for review successfully!";

					$username = $_COOKIE['username'];

					echo "<meta http-equiv='refresh' content='3; url=gamez?username=$username' />";

				}

			}

			if (isset($_GET['game_deleted'])) {

				if ($_GET['game_deleted'] == "success") {

					$gameDeletedSuccessfully = "Your game has been deleted successfully!";
			...
			?>
				...
				<h1 id="gamez_label_2">My Gamez</h1>

				<p id="game_edited_successfully"><?php echo $gameEditedSuccessfully; ?></p>

				<p id="game_deleted_successfully"><?php echo $gameDeletedSuccessfully; ?></p>

				<p id="number_of_gamez_2"><?php echo $gameCounter . " ( " . $pendingGameCounter . " ) "; ?></p>

				<p id="gamez_not_found_2"><?php echo $noGames; ?></p>
...
<?php if ($login == 1 && $selectGameQueryResult->num_rows > 0) : ?>
	...
	<?php
						...
						if ($gameActiveStatus == 1) {
							...
							?>
								...
								<style type="text/css">
								    ...
								    .gamez_for_others:hover #gamez_image_for_others {
							...
							$gamesForOthers = $row['gameName'];
							$gameID = $row['gameId'];
							$gamesListForOthers .= "<li class='gamez_for_others' onclick=\"window.location.href='/game?id=" . $gameID . "'\">
			
														<div id='gamez_image_for_others' class='gamez_image_for_others' style='background-image: url(" . $gameCoverImage . ");'>
															
															<button id='gamez_button_for_others' onclick='window.location.href = '#' '></button>

														</div>

														<div id='gamez_title_div_for_others'>

															<p id='gamez_title_for_others'>" . $gamesForOthers . "</p>
		...
		if ($selectNumberOfGamesQueryResult->num_rows >= 1) {
			...
			$noGamesForOthers = $username . " 's gamez list is currently empty!'";
			$gameCounter = "0 gamez active";
			$pendingGameCounter = "0 gamez pending";
			$errors = 1;
	...
	<div id="gamez_label_for_others_div">
		...
		<h1 id="gamez_label_for_others"><?php echo $username; ?></h1><h1 id="gamez_label_for_others_2">'s Gamez</h1>

	</div>

	<p id="number_of_gamez_for_others"><?php echo $gameCounter . " ( " . $pendingGameCounter . " ) "; ?></p>

	<p id="gamez_not_found_for_others"><?php echo $noGamesForOthers; ?></p>
		
	<?php echo $gamesListForOthers; ?>
...
<?php if ($login == 0 && $selectGameQueryResult->num_rows > 0) : ?>
...
<?php if ($login == 0 && $selectGameQueryResult->num_rows == 0) : ?>
	...
	<?php
		...
		if (isset($_GET['username'])) {
			...
			if ($selectGameQueryResult->num_rows == 0) {
		...
		if (isset($_GET['username'])) {
			...
			if (isset($_GET['username']) && $result->num_rows > 0 && $_GET['username'] == $_COOKIE['username']) {
				...
				?>
					...
					<h1 id="gamez_label">My Gamez</h1>

					<p id="number_of_gamez"><?php echo $gameCounter . " ( " . $pendingGameCounter . " ) "; ?></p>

					<p id="gamez_not_found"><?php echo $noGames; ?></p>
...
<script type="text/javascript">
	...
	setTimeout(function() {
		...
		$("#game_edited_successfully").fadeOut(300);

	}, 2000);

	setTimeout(function() {

		$("#game_deleted_successfully").fadeOut(300);
