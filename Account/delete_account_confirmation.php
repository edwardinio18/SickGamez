<?php
	...
	$deleteAccountError = "";
	$emptySelectorOrValidatorError = "";
	$invalidSelectorOrToken = "";
	$URLError = "";

	if (isset($_POST['delete_account_confirmation_submit'])) {
		...
		if (empty($email)) {
			...
			$sql = "SELECT deleteAccountEmail FROM deleteAccount WHERE deleteAccountSelector = ? AND deleteAccountToken = ?;";
			$stmt = $db->prepare($sql);
			$selector = $_GET["selector"];
			$validator = $_GET["validator"];
			$stmt->bind_param("ss", $selector, $validator);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($deleteAccountEmail);

			if ($stmt->num_rows > 0) {

				if ($stmt->fetch()) {

					$accountEmail = $deleteAccountEmail;
					...
					if ($accountEmail == $email) {
						...
						$emailDoesNotExist = "The email address entered does not match the email that is linked with the account deletion request!";
						$errors = 1;
		...
		if ($errors == 0) {
			...
			$sql = "SELECT deleteAccountSelector, deleteAccountToken, deleteAccountEmail, deleteAccountUsername, deleteAccountUserId FROM deleteAccount WHERE deleteAccountSelector = ? AND deleteAccountToken = ?;";
			$stmt = $db->prepare($sql);
			$selector = $_GET["selector"];
			$validator = $_GET["validator"];
			$stmt->bind_param("ss", $selector, $validator);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($deleteAccountSelector, $deleteAccountToken, $deleteAccountEmail, $deleteAccountUsername, $deleteAccountUserId);
			$stmt->fetch();

			if ($selector == $deleteAccountSelector && $validator == $deleteAccountToken) {

				$sql = "SELECT email, id FROM users WHERE email = ?;";
				$stmt = $db->prepare($sql);
				$tokenEmail = $deleteAccountEmail;
				$stmt->bind_param("s", $tokenEmail);
				$stmt->execute();
				$stmt->store_result();
				$stmt->bind_result($email, $myID);
				$stmt->fetch();

				if ($tokenEmail == $email) {

					$sql = "DELETE FROM deleteAccount WHERE deleteAccountEmail = ?;";
					...
					if (!mysqli_stmt_prepare($stmt, $sql)) {
						...
						$sql = "DELETE FROM friendRequests WHERE fromUserId = ? OR toUserId = ?;";
						$stmt = mysqli_stmt_init($db);
						...
						if (!mysqli_stmt_prepare($stmt, $sql)) {
							...
							mysqli_stmt_bind_param($stmt, "ii", $deleteAccountUserId, $deleteAccountUserId);
							mysqli_stmt_execute($stmt);

							$sql = "DELETE FROM friends WHERE friend1Id = ? OR friend2Id = ?;";
							$stmt = mysqli_stmt_init($db);
							...
							if (!mysqli_stmt_prepare($stmt, $sql)) {
								...
								$sql = "DELETE FROM gameComments WHERE gameCommentUserId = ?;";
								$stmt = mysqli_stmt_init($db);
								...
								if (!mysqli_stmt_prepare($stmt, $sql)) {
									...
									mysqli_stmt_bind_param($stmt, "i", $deleteAccountUserId);
									mysqli_stmt_execute($stmt);

									$sql = "DELETE FROM gameLikesDislikes WHERE userIdLikesDislikesGames = ?;";
									$stmt = mysqli_stmt_init($db);
									...
									if (!mysqli_stmt_prepare($stmt, $sql)) {
										...
										$selectGameForUserQuery = "SELECT * FROM gamez WHERE username = ?;";
										$stmt = $db->prepare($selectGameForUserQuery);
										$stmt->bind_param("s", $deleteAccountUsername);
										$stmt->execute();
										$selectGameForUserQueryResult = $stmt->get_result();

										if ($selectGameForUserQueryResult->num_rows >= 1) {

											while ($row = $selectGameForUserQueryResult->fetch_assoc()) {
										...
										$sql = "DELETE FROM gamez WHERE username = ?;";
										$stmt = mysqli_stmt_init($db);
										...
										if (!mysqli_stmt_prepare($stmt, $sql)) {
											...
											mysqli_stmt_bind_param($stmt, "s", $deleteAccountUsername);
											mysqli_stmt_execute($stmt);

											$sql = "DELETE FROM messages WHERE fromUserIdMessage = ? OR toUserIdMessage = ?;";
											$stmt = mysqli_stmt_init($db);
											...
											if (!mysqli_stmt_prepare($stmt, $sql)) {
												...
												if (!mysqli_stmt_prepare($stmt, $sql)) {
													...
													if (!mysqli_stmt_prepare($stmt, $sql)) {
														...
														$stmt = $db->prepare($selectProfilePictureQuery);
														$stmt->bind_param("s", $deleteAccountUsername);
														$stmt->execute();
														$selectProfilePictureQueryResult = $stmt->get_result();
														$row = $selectProfilePictureQueryResult->fetch_assoc();

														$currentCustomBackgroundProfileImage = $row['backgroundProfileImage'];
														$currentCustomBackgroundProfileImagePath = '/home/yv4nbnligki0/public_html/Background_profile_images/' . $currentCustomBackgroundProfileImage;

														$currentCustomNotificationsSound = $row['notificationsSound'];
														$currentCustomNotificationsSoundPath = '/home/yv4nbnligki0/public_html/Notifications_sounds/' . $currentCustomNotificationsSound;

														unlink($currentCustomBackgroundProfileImagePath);
														unlink($currentCustomNotificationsSoundPath);

														$sql = "DELETE FROM shopProductsUsersList WHERE shopProductsUsersListUserId = ?;";
														$stmt = mysqli_stmt_init($db);
														...
														if (!mysqli_stmt_prepare($stmt, $sql)) {
															...
															$sql = "DELETE FROM xubeCoinData WHERE xubeCoinDataUserId = ?;";
															$stmt = mysqli_stmt_init($db);
															...
															if (!mysqli_stmt_prepare($stmt, $sql)) {
																...
																$sql = "DELETE FROM xubeGameData WHERE xubeGameDataUsername = ?;";
																$stmt = mysqli_stmt_init($db);
																...
																if (!mysqli_stmt_prepare($stmt, $sql)) {
																	...
																	$selectProfilePictureQuery = "SELECT * FROM users WHERE username = ?;";
																	$stmt = $db->prepare($selectProfilePictureQuery);
																	$stmt->bind_param("s", $deleteAccountUsername);
																	$stmt->execute();
																	$selectProfilePictureQueryResult = $stmt->get_result();
																	...
																	$currentProfilePicture = $row['profileImage'];
																	$currentProfilePicturePath = '/home/yv4nbnligki0/public_html/Profile_pictures/' . $currentProfilePicture;
																	...
																	if (!mysqli_stmt_prepare($stmt, $sql)) {
																		...
																		header("location: log_out_delete_account_confirmation");
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Delete your SickGamez account confirmation!" />
	...
	<style type="text/css">
	    ...
	    #delete_account_confirmation_submit {
	    ...
	    #delete_account_confirmation_submit:active {
	    ...
	    #delete_account_sorry_to_see_you_go_label {
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<h1 id="delete_account_confirmation">Confirm account deletion</h1>

	<p id="delete_account_sorry_to_see_you_go_label">We're sorry to see you go but we hope that in the promising future we shall see your return back on the surfaces of SickGamez! We wish you all the best as you continue your journey on the mysterious paths of the unknown! So long gamer...</p>
	...
	<?php
		...
		if (empty($_GET['selector']) || empty($_GET['validator'])) {
			...
			$emptySelectorOrValidatorError = "The URL selector or validator are either empty or have been modified!";

			?>

				<h1 id="delete_account_error"><?php echo $deleteAccountError; ?></h1>

				<p id="empty_selector_or_validator"><?php echo $emptySelectorOrValidatorError; ?></p>

				<script type="text/javascript">
					
					$(document).ready(function() {

						$("#delete_account_confirmation").remove();
						$("#delete_account_sorry_to_see_you_go_label").remove();
			...
			$deleteAccountQuery = "SELECT deleteAccountSelector, deleteAccountToken FROM deleteAccount WHERE deleteAccountSelector = ? AND deleteAccountToken = ?;";
			...
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($deleteAccountSelector, $deleteAccountToken);
			$stmt->fetch();
			...
			if ($selector == $deleteAccountSelector && $validator == $deleteAccountToken) {
				...
				?>
					...
					<form action="delete_account_confirmation?selector=<?php echo $selector ?>&validator=<?php echo $validator ?>" method="post" id="delete_account_confirmation_form" style="text-align: center; margin-top: 5%">

						<input type="hidden" name="selector" value="<?php echo $selector ?>;" />

						<input type="hidden" name="validator" value="<?php echo $validator ?>;" />

						<p id="email_required"><?php echo $emailError; ?></p>

						<p id="email_does_not_exist"><?php echo $emailDoesNotExist; ?></p>

						<input type="email" name="email" placeholder="Email address" class="formInputs" id="emailaddress" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

						<br />
						
						<input type="submit" name="delete_account_confirmation_submit" id="delete_account_confirmation_submit" value="Confirm my email address and delete my account!" />
				...
				$deleteAccountError = "Error";
				$invalidSelectorOrToken = "The URL selector or validator is not valid!";

				?>

					<h1 id="delete_account_error"><?php echo $deleteAccountError; ?></h1>

					<p id="invalid_selector_or_token"><?php echo $invalidSelectorOrToken; ?></p>
