...
<head>
	...
	<style type="text/css">
		...
		#friend_requests_not_accessible {
	    ...
	    .friend_request_profile_pic_display {
	    ...
	    .friend_request_from_user_date_time {
	    ...
	    #friend_requests_list_div {

	    	margin-top: 5%;

	    }

	    #friend_request_counter {
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<?php if ($login == 1) : ?>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#friend_requests_error").remove();
				$("#friend_requests_not_accessible").remove();
		...
		<?php
			...
			$query = "SELECT * FROM friendRequests WHERE toUserId = ?;";
							...
							if (!mysqli_stmt_prepare($stmt, $query)) {
								...
								if (!mysqli_stmt_prepare($stmt, $query)) {
									...
									if (!mysqli_stmt_prepare($stmt, $query)) {
										...
										echo "<meta http-equiv='refresh' content='0; url=friend_requests' />";
		...
		<h1 id="friend_requests_label">Friend requests</h1>

		<div id="friend_requests_list_div"></div>
	...
	<?php if ($login == 0) : ?>
		...
		<h1 id="friend_requests_error"><?php $profileAndUserError = "Error"; echo $profileAndUserError; ?></h1>

		<p id="friend_requests_not_accessible"><?php $friendRequestsNotAccessible = "Friend requests can't be accessed if not logged in!"; echo $friendRequestsNotAccessible; ?></p>
			...
			setInterval(function() {
				...
				xhttp.onreadystatechange = function() {
					...
					if (this.readyState == 4 && this.status == 200) {
						...
						document.getElementById("friend_requests_list_div").innerHTML = this.responseText;

					}

				};

				xhttp.open("GET", "friend_request_page_ajax_refresh.php", true);
        		...
        		$.ajax({
	        		...
	        		success: function(data) {
	        			...
	        			if (data != "") {
	        				...
	        				var visibleFriendRequestContainer = $(".friend_request_div").is(":visible");

				        	if (visibleMessageContainer || visibleFriendRequestContainer) {
