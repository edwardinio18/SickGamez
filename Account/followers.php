<?php
	...
	$profileNotAccessible = "Followers can't be accessed!";
	$profileAndUserError = "Error";

	$myFollowersList = "";
	$followersCounter = 0;
	$myFollowersListForOthers = "";
	$followersListEmptyForOthers = "";
	$followersListEmpty = "";
...
<head>
	...
	<style type="text/css">
	    ...
	    #number_of_followers_for_others {
	    ...
	    #followers_label_for_others_div {
					...
					?>
						...
						<script type="text/javascript">
							...
							$(document).ready(function() {
								...
								$("#followers_label_for_others").remove();
								$("#followers_label_for_others_2").remove();
								$("#number_of_followers_for_others").remove();
								$("#no_followers_found_for_others").remove();
								$(".followers_div_for_others").remove();
...
<?php
		...
		if ($result->num_rows >= 1) {
			...
			if ($numberOfFollowersQueryResult->num_rows >= 1) {
				...
				while ($row = $numberOfFollowersQueryResult->fetch_assoc()) {
					...
					$selectFollowerDataQuery = "SELECT * FROM followers INNER JOIN users ON followers.followerUserId = users.id ORDER BY dateAndTimeStartedFollowing DESC;";
					...
					if ($selectFollowerDataQueryResult->num_rows >= 1) {
						...
						while ($row = $selectFollowerDataQueryResult->fetch_assoc()) {
							...
							if ($myID === $row['followingUserId']) {
								...
								$followerUsernameIdDB = "follower_username_" . $followerUsernameId;
								...
								$customUsernameStylingTextId = 2;
								$customUsernameStylingTextActiveStatus = 1;
								$stmt->bind_param("sii", $followerUsernameId, $customUsernameStylingTextId, $customUsernameStylingTextActiveStatus);
								$stmt->execute();
								$checkCustomUsernameStylingTextActiveStatusQueryResult = $stmt->get_result();
								...
								$myFollowersList .= "<div class='followers_div'>
														...
														<div class='follower_username_div'>
															...
															<a href='profile?username=" . $followerUsername . "' class='follower_user_link'>
																...
																<p class='follower_username' id='follower_username_" . $followerUsernameId . "'>" . $followerUsername . "</p>
														...
														<form action='' method='post'>
															...
															<input type='hidden' name='id' value='" . $followerUsernameId . "' />
													
															<input type='submit' name='remove_follower' id='remove_follower' value='Remove follower!' />
	...
	if (isset($_POST['remove_follower'])) {
		...
		if ($result->num_rows >= 1) {
			...
			$selectRemoveFollowerQuery = "SELECT * FROM followers WHERE followingUserId = ? AND followerUserId = ?;";
			$stmt = $db->prepare($selectRemoveFollowerQuery);
			$stmt->bind_param("ii", $myID, $userID);
			$stmt->execute();
			$selectRemoveFollowerQueryResult = $stmt->get_result();

			if ($selectRemoveFollowerQueryResult->num_rows >= 1) {

				if ($row = $selectRemoveFollowerQueryResult->fetch_assoc()) {

					$query = "DELETE FROM followers WHERE followingUserId = ? AND followerUserId = ?;";
					...
					if (!mysqli_stmt_prepare($stmt, $query)) {
						...
						echo "<meta http-equiv='refresh' content='0; url=followers?username=$username' />";
...
<h1 id="followers_label">My Followers</h1>
...
<?php if ($login == 1 && $numberOfFollowersQueryResult->num_rows >= 1) : ?>
	...
	<?php
					...
					if ($selectFollowerDataQueryResult->num_rows >= 1) {
						...
						while ($row = $selectFollowerDataQueryResult->fetch_assoc()) {
							...
							if ($userID === $row['followingUserId']) {
								...
								$followerUsernameIdDB = "follower_username_for_others_" . $followerUsernameId;
								...
								$myFollowersListForOthers .= "<div class='followers_div_for_others'>
			
																<img src='" . $userProfilePictureFollowerPath . "' class='profile_pic_display_followers_for_others' />

																<div class='follower_username_div_for_others'>
																	
																	<a href='profile?username=" . $followerUsername . "' class='follower_user_link_for_others'>

																		<p class='follower_username_for_others' id='follower_username_for_others_" . $followerUsernameId . "'>" . $followerUsername . "</p>
		...
		if ($result->num_rows >= 1) {
			...
			if ($numberOfFollowersQueryResult->num_rows >= 1) {
				...
				$followersCounter = "0 followers";
				$errors = 1;
				$followersListEmptyForOthers = $username . "'s followers list is currently empty!";
	...
	<div id="followers_label_for_others_div">
		...
		<h1 id="followers_label_for_others"><?php echo $username; ?></h1><h1 id="followers_label_for_others_2">'s Followers</h1>

	</div>

	<p id="number_of_followers_for_others"><?php echo $followersCounter; ?></p>

	<p id="no_followers_found_for_others"><?php echo $followersListEmptyForOthers; ?></p>
...
<?php if ($login == 0 && $numberOfFollowersQueryResult->num_rows >= 1) : ?>
...
<?php if ($login == 0 && $numberOfFollowersQueryResult->num_rows == 0) : ?>
...
<?php if ($login == 1 && $numberOfFollowersQueryResult->num_rows == 0) : ?>
