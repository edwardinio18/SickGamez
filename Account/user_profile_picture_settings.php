...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Modify your profile picture settings here!" />
	...
	<style type="text/css">
	    ...
	    #delete_submit:active {

	    	background-color: white;

	    }

	    #profile_picture_changed_successfully {
	    ...
	    #user_profile_picture_settings_error {
		...
		#user_profile_picture_settings_not_accessible {
	    ...
	    .croppie-container .cr-vp-circle:focus {
	    ...
	    .cr-slider:hover, .cr-slider:active {
	    ...
	    #webcam_picture_button:active {

	    	background-color: white;

	    }

	    #webcam_picture_button_div {

	    	text-align: center;

	    }

	    #webcam_picture_button_image {
	    ...
	    #webcam_picture_button_image_span {
	    ...
	    #webcam_picture_button_text_span {
	    ...
	    #webcam_picture_button:active > #webcam_picture_button_image_span {
	    ...
	    .close_fullscreen_webcam_div_modal_span {
	    ...
	    .close_fullscreen_webcam_div_modal_span:hover, .close_fullscreen_webcam_div_modal_span:focus {
	    ...
	    #webcame_picture_live_div {

	    	text-align: center;

	    }

	    #webcam_picture_button_live {
	    ...
	    #webcam_picture_button_live:active {

	    	background-color: white;

	    }

	    #webcam_image_boundary_container {
	    ...
	    #webcam_video_camera_denied_access_modal_container {
	    ...
	    #webcam_video_camera_denied_access_modal_content {
	    ...
	    .close_webcam_video_camera_denied_access_modal {
	    ...
	    .close_webcam_video_camera_denied_access_modal:hover, .close_webcam_video_camera_denied_access_modal:focus {
	    ...
	    #webcam_video_camera_denied_access_inner_modal_title {
	    ...
	    #webcam_video_camera_denied_access_inner_modal_title_2 {
	    ...
	    .webcam_video_camera_denied_access_button {
	    ...
	    .webcam_video_camera_denied_access_button:active {
...
<?php
	...
	if ($login == 1) {
		...
		$noCurrentProfilePicture = "";
		$profilePictureChangedSuccessfully = "";
		$profilePictureDeleted = "";

		if (isset($_POST['delete_profile_picture_submit'])) {

			if ($row = $result->fetch_assoc()) {

				$userProfilePicture = $row["profileImage"];

				if ($userProfilePicture == "") {

					$userProfilePicture = "Images/profile_image_placeholder.png";
					$noCurrentProfilePicture = "You need to have an active profile picture to be able to delete it!";
					$errors = 1;
			...
			if ($errors == 0) {
				...
				$updateQuery = "UPDATE users SET profileImage = ? WHERE username = ?;";
				...
				if (!mysqli_stmt_prepare($stmt, $updateQuery)) {
					...
					mysqli_stmt_bind_param($stmt, "ss", $profileImage, $username);
					...
					echo "<meta http-equiv='refresh' content='0; url=user_profile_picture_settings?profile_picture_deleted=success' />";
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<?php if ($login == 1) : ?>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#user_profile_picture_settings_error").remove();
				$("#user_profile_picture_settings_not_accessible").remove();

			})

		</script>

	<h1 id="user_profile_picture_settings">Profile picture settings</h1>

	<?php

		if (isset($_GET['profile_picture_changed'])) {

			if ($_GET['profile_picture_changed'] == "success") {

				$profilePictureChangedSuccessfully = "Your profile picture has been changed successfully!";

				echo "<meta http-equiv='refresh' content='3; url=user_profile_picture_settings' />";

			}

		}

		if (isset($_GET['profile_picture_deleted'])) {

			if ($_GET['profile_picture_deleted'] == "success") {

				$profilePictureDeleted = "Your profile picture has been deleted!";
	...
	<p id="profile_picture_changed_successfully"><?php echo $profilePictureChangedSuccessfully; ?></p>

	<p id="profile_picture_deleted"><?php echo $profilePictureDeleted; ?></p>

	<div id="croppie_div">
		
		<div class="" id="uploaded_image"></div>

		<button class="button_preview_image" style="cursor: default;">Upload image!</button>

		<p id="no_image_uploaded"></p>

	</div>

	<form method="post" action="" enctype="multipart/form-data" style="text-align: center; margin-top: 5%;">

		<p id="inputError"><?php echo $noProfilePictureSelected; ?></p>

		<p id="inputError2"><?php echo $noCurrentProfilePicture; ?></p>

		<img src="<?php echo $userProfilePicture; ?>" id="profile_pic_display" onclick="profileImage();" />

		<input type="file" id="profile_pic" class="profile_pic" name="profile_pic" accept=".png, .jpg, jpeg" />

		<br />

		<input type="submit" name="change_profile_picture_submit" id="submit" value="Change profile picture!" />

		<br />

		<input type="submit" name="delete_profile_picture_submit" id="delete_submit" value="Delete profile picture!" />

	</form>

	<div id="webcam_picture_div"></div>
	
	<div id="webcam_picture_button_div">
	
		<button name="webcam_picture_button" id="webcam_picture_button" class="webcam_picture_button">

			<span id="webcam_picture_button_image_span"><img src="Images/camera.png" id="webcam_picture_button_image" /></span>

			<span id="webcam_picture_button_text_span">Take a picture now!</span>
	...
	<div id="back_arrow_div">
		...
		<a href="account_settings" id="back_arrow_link">
	...
	<?php if ($login == 0) : ?>
		...
		<h1 id="user_profile_picture_settings_error"><?php $userProfilePictureSettingsError = "Error"; echo $userProfilePictureSettingsError; ?></h1>

		<p id="user_profile_picture_settings_not_accessible"><?php $userProfilePictureSettingsError = "User profile picture settings can't be accessed if not logged in!"; echo $userProfilePictureSettingsError; ?></p>
...
<script type="text/javascript">
	...
	var resize = $('#uploaded_image').croppie({
	...
	$('.button_preview_image').on('click', function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (image.files.length == 0) {
						...
						var noImageSelected = document.getElementById("no_image_uploaded");

						noImageSelected.innerHTML = "No image has been uploaded!";

					} else {

						document.querySelector("#profile_pic_display").setAttribute('src', img);

						$(document).ready(function() {

							$("#no_image_uploaded").remove();
	...
	$("#submit").on("click", function(e) {
		...
		if (image.files.length == 0) {
			...
			var noImageSelected = document.getElementById("inputError");

			noImageSelected.innerHTML = "No image has been selected!";
	...
	setTimeout(function() {
		...
		$("#profile_picture_changed_successfully").fadeOut(300);

	}, 2000);

	setTimeout(function() {

		$("#profile_picture_deleted").fadeOut(300);
    ...
    Webcam.attach('#webcam_picture_div');
	...
	$(document).on("click", ".webcam_picture_button", function() {

		$("video").css("display", "block");
		$("body").css("overflow", "hidden");
		$("html").css("overflow", "hidden");
		$("#webcam_picture_div").append("<span class='close_fullscreen_webcam_div_modal_span'>&times;</span>");
		$("body").append('<div id="webcame_picture_live_div"><button name="webcam_picture_button_live" onclick="takePicture();" id="webcam_picture_button_live" class="webcam_picture_button_live">Snap your picture!</button></div>');
		$("body").append('<span id="webcam_image_boundary_container"></span>');

	});

	$(document).on("click", ".close_fullscreen_webcam_div_modal_span", function() {

		$("video").hide();
		$(this).remove();
		$("#webcame_picture_live_div").remove();
		$("body").css("overflow", "auto");
		$("html").css("overflow", "auto");
		$("#webcam_image_boundary_container").remove();

	});

	$(document).on("click", "#webcam_picture_button_live", function() {

		$(".close_fullscreen_webcam_div_modal_span").remove();
		$("video").hide();
		$("#webcame_picture_live_div").remove();
		$("body").css("overflow", "auto");
		$("html").css("overflow", "auto");
	...
	$(document).on("click", ".close_webcam_video_camera_denied_access_modal", function() {

		$("#webcam_video_camera_denied_access_modal_container").remove();
		document.getElementById("webcam_picture_button").disabled = true;
		$("#webcam_picture_button").css("opacity", "0.5");

	});

	$(document).on("click", ".webcam_video_camera_denied_access_button", function() {
