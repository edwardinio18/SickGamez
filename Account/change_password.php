<?php
	...
	$currentPasswordSameAsNewPassword = "";
	...
	if (isset($_POST['changepasswordsubmit'])) {
		...
		if (empty($confirmNewPassword)) {
			...
			$confirmNewPasswordEmpty = "Confirm new password is required!";
			$errors = 1;
		...
		if ($errors == 0) {
			...
			$currentPasswordDB = $password;

			if (password_verify($currentPassword, $currentPasswordDB)) {

				if ($newPassword == $confirmNewPassword) {

					if (!password_verify($newPassword, $currentPasswordDB)) {

						$newPassword = password_hash($newPassword, PASSWORD_DEFAULT);

						$changePassword = "UPDATE users SET password = ? WHERE username = ?;";
						$stmt = mysqli_stmt_init($db);

						if (!mysqli_stmt_prepare($stmt, $changePassword)) {
			 			...
			 			$passwordChangedSuccesfully = "Your password has been changed succesfully! You will now be redirected back to the login page!";

			 			session_destroy();
			 			unset($_COOKIE['username']);
			 			setcookie("username", ' ', time() - (60 * 60 * 24), '/');
			 			setcookie("login", 0, time() - (60 * 60 * 24), '/');
			 			header("refresh:3; url=log_in");

					} else {

						$currentPasswordSameAsNewPassword = "Your new password can't be the same as your current password!";
						$errors = 1;
			 		...
			 		$newPasswordsNotMatching = "New passwords must match!";
			 		$errors = 1;
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Change your SickGamez account password!" />
	...
	<style type="text/css">
		...
		#change_password_not_accessible {
	    ...
	    #current_password_and_new_password_same_error {
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<?php if ($login == 1) : ?>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#change_password_not_accessible").remove();

			})

		</script>

		<h1 id="change_password_label">Change password</h1>

		<p id="password_changed_succesfully"><?php echo $passwordChangedSuccesfully; ?></p>

		<form action="" method="post" id="change_password_form" style="text-align: center; margin-top: 6.5%;">

			<p id="inputError"><?php echo $currentPasswordEmpty; ?></p>

			<p id="current_password_incorrect"><?php echo $currentPasswordIncorrect; ?></p>

			<p id="current_password_and_new_password_same_error"><?php echo $currentPasswordSameAsNewPassword; ?></p>
			
			<input type="password" name="currentpassword" placeholder="Current password" class="formInputs" id="currentpassword" maxlength="30" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

			<br />

			<p id="inputError"><?php echo $newPasswordEmpty; ?></p>

			<input type="password" name="newpassword" placeholder="New password" class="formInputs" id="newpassword" maxlength="30" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

			<p id="new_passwords_not_matching"><?php echo $newPasswordsNotMatching; ?></p>

			<br />

			<p id="inputErrorLong"><?php echo $confirmNewPasswordEmpty; ?></p>

			<input type="password" name="confirmnewpassword" placeholder="Confirm new password" class="formInputs" id="confirmnewpassword" maxlength="30" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

			<br />

			<input type="submit" name="changepasswordsubmit" id="submit" value="Change password!" />
	...
	<?php if ($login == 0) : ?>
		...
		<h1 id="change_password_error"><?php $profileAndUserError = "Error"; echo $profileAndUserError; ?></h1>

		<p id="change_password_not_accessible"><?php $changePasswordNotAccessible = "You can't change your password unless you are logged in!"; echo $changePasswordNotAccessible; ?></p>
