<?php
    ...
    $deleteAccountEmailSentSuccessfully = "";
    $redirectToHomePage = "";
    ...
    if (isset($_POST['delete_account_submit'])) {
    		...
    		if ($stmt->num_rows > 0) {
    			...
    			if ($stmt->fetch()) {
    				...
    				if ($accountEmail == $email) {
    					...
    					$emailDoesNotExist = "The email address entered does not match this current account's email!";
    					$errors = 1;
    	...
    	if ($errors == 0) {
    		...
    		$url = "https://www.sickgamez.com/Account/delete_account_confirmation?selector=" . $selector . "&validator=" . $token;
    		...
    		$sql = "INSERT INTO deleteAccount (deleteAccountUserId, deleteAccountUsername, deleteAccountEmail, deleteAccountSelector, deleteAccountToken, deleteAccountExpires) VALUES (?, ?, ?, ?, ?, ?);";
    		$stmt = mysqli_stmt_init($db);
    		...
    		if (!mysqli_stmt_prepare($stmt, $sql)) {
    			...
    			mysqli_stmt_bind_param($stmt, "isssss", $myID, $username, $email, $selector, $token, $expires);
    			mysqli_stmt_execute($stmt);
    		...
    		$to = $email;
            $subject = "Delete your SickGamez account";
            $message = '<html><body>';
            $message .= '<p>We have received a request that you would like to delete your SickGamez account! Click the link below to confirm the permanent deletion of your account! If this action was not requested by you, you can ignore this email!</p>';
            $message .= '<br />';
            $message .= '<p>';
            $message .= '<a href="' . $url . '">Delete my SickGamez account</a>';
            $message .= '</p>';
    		...
    		$deleteAccountEmailSentSuccessfully = "An account deletion confirmation email has been sent out! Check your inbox!";
    		$redirectToHomePage = "You will now be logged out and redirected to the homepage!";
    
    		header("refresh:4; url=log_out");
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Delete your SickGamez account!" />
	...
	<style type="text/css">
	    ...
	    #delete_account_confirmation_email {
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<?php if ($login == 1) : ?>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#delete_account_error").remove();
				$("#delete_account_not_accessible").remove();

			})

		</script>

		<h1 id="delete_account">Delete my account</h1>

		<p id="delete_account_confirmation_email"><?php echo $deleteAccountEmailSentSuccessfully; ?></p>

		<p id="redirect_to_home_page"><?php echo $redirectToHomePage; ?></p>

		<form action="" method="post" id="delete_account_form" style="text-align: center; margin-top: 9%;">
			...
			<input type="submit" name="delete_account_submit" id="delete_account_submit" value="Send account deletion confirmation email!" />
	...
	<?php if ($login == 0) : ?>
		...
		<h1 id="delete_account_error"><?php $profileAndUserError = "Error"; echo $profileAndUserError; ?></h1>

		<p id="delete_account_not_accessible"><?php $deleteAccountNotAccessible = "You can't delete your account unless you are logged in!"; echo $deleteAccountNotAccessible; ?></p>
