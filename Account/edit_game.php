<?php
	...
	$noGameCoverImageUploaded = "";
	$editGameLoggedInDifferentUser = "";
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="This is the edit page for your gamez!" />
	...
	<style type="text/css">
		...
		#edit_game_not_accessible_error {
	    ...
	    #game_not_accessible_not_logged_in {
...
<?php
	...
	$newNameAndImageNotValid = "";
	$newNameAndImageNotValid2 = "";
	$nameDoesntExistButImageExistsError = "";
	$nameExistsButImageDoesntExistError = "";

	if (isset($_GET['id'])) {

		$query = "SELECT gameName, gameDescription, gameCoverImage FROM gamez WHERE gameId = ?;";
		...
		if ($result->num_rows >= 1) {
			...
			if ($row = $result->fetch_assoc()) {
				...
				$currentGameDescription = $row['gameDescription'];
				$currentGameCoverImage = $row['gameCoverImage'];
				$currentGameDescriptionDecrypted = decrypt($currentGameDescription, $key);
	...
	if (isset($_POST['edit_button'])) {
		...
		$query = "SELECT * FROM gamez WHERE gameId = ?;";
		$stmt = $db->prepare($query);
		$id = $_GET['id'];
		$stmt->bind_param("s", $id);
		$stmt->execute();
		...
		$trimmedNewGameName = trim($newGameName);
		$strippedTagsNewGameName = strip_tags($trimmedNewGameName);

		$newGameDescription = $_POST['gamedescription'];
		$trimmedNewGameDescription = trim($newGameDescription);
		$strippedTagsNewGameDescription = strip_tags($trimmedNewGameDescription);
		$encryptedNewGameDescription = encrypt($strippedTagsNewGameDescription, $key);

		$currentGameCoverImage = $row['gameCoverImage'];
		$currentGameCoverImagePath = "/home/yv4nbnligki0/public_html/Game_cover_images/" . $currentGameCoverImage;

		$newUploadedGameCoverImage = $_FILES['gamecoverimage']['name'];
		$newGameCoverImageSource = $_FILES['gamecoverimage']['tmp_name'];
		$newGameCoverImageType = $_FILES['gamecoverimage']['type'];
		$newGameCoverImageTargetPath = "/home/yv4nbnligki0/public_html/Game_cover_images/" . $newUploadedGameCoverImage;

		if (empty($strippedTagsNewGameName)) {

			$emptyGameNameError = "A game name is required!";
			$errors = 1;

		}

		if (empty($strippedTagsNewGameDescription)) {

			$emptyGameDescription = "A desciption for your game is required!";
			$errors = 1;
		...
		if ($errors == 0) {
			...
			$query = "SELECT * FROM gamez WHERE gameName = ? OR gameName = ? OR gameCoverImage = ? OR gameCoverImage = ?;";
			$stmt = $db->prepare($query);
			$stmt->bind_param("ssss", $strippedTagsNewGameName, $currentGameName, $newUploadedGameCoverImage, $currentGameCoverImage);
			$stmt->execute();
			$result = $stmt->get_result();
			...
			if ($result->num_rows >= 1) {
				...
				while ($row = $result->fetch_assoc()) {
					...
					if ($newUploadedGameCoverImage == "") {

						if ($strippedTagsNewGameName === $currentGameName) {

							// successful update query! - no uploaded image and new name is same as current name!

							$strippedTagsNewGameName = $currentGameName;
							$id = $_GET['id'];
							$activeStatus = 0;
							$newUploadedGameCoverImage = $currentGameCoverImage;

							$updateGameInfoQuery = "UPDATE gamez SET gameName = ?, gameDescription = ?, gameCoverImage = ?, activeStatus = ? WHERE gameId = ?;";
							$stmt = mysqli_stmt_init($db);

							if (!mysqli_stmt_prepare($stmt, $updateGameInfoQuery)) {
								...
								mysqli_stmt_bind_param($stmt, "sssss", $strippedTagsNewGameName, $encryptedNewGameDescription, $newUploadedGameCoverImage, $activeStatus, $id);
								...
								echo "<meta http-equiv='refresh' content='0; url=gamez?username=$username&game_edited=success' />";

							}

						} else {

							$query = "SELECT * FROM gamez WHERE gameName = ?;";
							$stmt = $db->prepare($query);
							$stmt->bind_param("s", $strippedTagsNewGameName);
							$stmt->execute();
							$result = $stmt->get_result();
							...
							if ($result->num_rows >= 1) {
								...
								if ($row = $result->fetch_assoc()) {
									...
									if ($row['gameName'] === $strippedTagsNewGameName) {

										// failed update query! - no uploaded image but name is already taken!

										$newNameAndImageNotValid = "Game name is already taken!";
										$errors = 1;
								...
								// successful update query! - no uploaded image and new name doesn't exist and isn't the same as current name!
					...
					} else if ($newUploadedGameCoverImage != "") {
						...
						$currentGameCoverImage = $row['gameCoverImage'];
						$currentGameCoverImagePath = "/Game_cover_images/" . $currentGameCoverImage;

						$newUploadedGameCoverImage = $_FILES['gamecoverimage']['name'];
						$newGameCoverImageSource = $_FILES['gamecoverimage']['tmp_name'];
						$newGameCoverImageType = $_FILES['gamecoverimage']['type'];
						$newGameCoverImageTargetPath = "/Game_cover_images/" . $newUploadedGameCoverImage;

						if ($newUploadedGameCoverImage === $currentGameCoverImage) {

							if ($strippedTagsNewGameName === $currentGameName) {

								// successful update query! - new name is same as current name and new image is same as current image!
								...
								if ($result->num_rows >= 1) {
									...
									if ($row = $result->fetch_assoc()) {
										...
										if ($row['gameName'] === $strippedTagsNewGameName) {
											...
											// failed update query! - new name already exists and is different than current name and new image is same as current image!
									...
									// successful update query! - new name doesn't exist and new image is same as current image!
							...
							$query = "SELECT * FROM gamez WHERE gameCoverImage = ?;";
							$stmt = $db->prepare($query);
							$stmt->bind_param("s", $newUploadedGameCoverImage);
							$stmt->execute();
							$result = $stmt->get_result();
							...
							if ($result->num_rows >= 1) {
								...
								if ($row = $result->fetch_assoc()) {
									...
									if ($row['gameCoverImage'] === $newUploadedGameCoverImage) {

										if ($strippedTagsNewGameName === $currentGameName) {

											// failed update query! - new name is same as current name and new image already exists and is different than current image!

											$nameDoesntExistButImageExistsError = "Game cover image with the same filename is already taken!";
											$errors = 1;
											...
											if ($result->num_rows >= 1) {
												...
												if ($row = $result->fetch_assoc()) {
													...
													if ($row['gameName'] === $strippedTagsNewGameName) {
														...
														// failed update query! - new name already exists and is different than current name and new image already exists and is different than current image!

														$newNameAndImageNotValid2 = "Game name and game cover image with the same filename are already taken!";
														$errors = 1;
												...
												// failed update query! - new name doesn't exist and new image already exists and is different than current image!
								...
								if ($strippedTagsNewGameName === $currentGameName) {
									...
									// successful update query! - new name is same as current name and new image doesn't exist!
									...
									unlink($currentGameCoverImagePath);
									...
									if (!mysqli_stmt_prepare($stmt, $updateGameInfoQuery)) {
										...
										move_uploaded_file($newGameCoverImageSource, $newGameCoverImageTargetPath);
									...
									if ($result->num_rows >= 1) {
										...
										if ($row = $result->fetch_assoc()) {
											...
											if ($row['gameName'] === $strippedTagsNewGameName) {
												...
												// failed update query! - new name already exists and is different than current name and new image doesn't exist!
										...
										// successful update query! - new name doesn't exist and new image doesn't exist!
	...
	if (isset($_POST['delete_button'])) {
		...
		$gameFolderPath = "/home/yv4nbnligki0/public_html/Games/" . $row['gameFile'];
		$gameFolderPathInfo = pathinfo($gameFolderPath);
		$gameFileName = $gameFolderPathInfo['filename'];
		$gameFolderFinalPath = "/home/yv4nbnligki0/public_html/Games/" . $gameFileName . "/";

		deleteFiles($gameFolderFinalPath);
		...
		$deleteGameQuery = "DELETE FROM gamez WHERE username = ? AND gameId = ?;";
		$stmt = mysqli_stmt_init($db);

		if (!mysqli_stmt_prepare($stmt, $deleteGameQuery)) {
			...
			echo "<meta http-equiv='refresh' content='0; url=gamez?username=$username&game_deleted=success' />";
...
<h1 id="edit_game_label">Edit game</h1>

<div class="formDiv">

	<form action="" method="post" style="text-align: center;" enctype="multipart/form-data">

		<p id="inputError"><?php echo $emptyGameNameError; ?></p>

		<p id="inputError3"><?php echo $newNameAndImageNotValid; ?></p>

		<p id="inputError4"><?php echo $nameExistsButImageDoesntExistError; ?></p>

		<input type="text" name="gamename" value="<?php echo $currentGameName; ?>" class="formInputs" id="gamename" onfocus="this.placeholder = ''" onblur="this.placeholder = ''" <?php if ($errors == 1 && empty($strippedTagsNewGameName)) echo "style='border: 2.5px solid red;'"; ?> />

		<br />

		<p id="inputError2"><?php echo $emptyGameDescription; ?></p>

		<p id="inputError5"><?php echo $newNameAndImageNotValid2; ?></p>

		<textarea name="gamedescription" id="gamedescription" maxlength="300" placeholder="<?php echo $currentGameDescriptionDecrypted; ?>" onfocus="this.placeholder = ''" onblur="this.placeholder = ''" <?php if ($errors == 1 && empty($strippedTagsNewGameDescription)) echo "style='border: 2.5px solid red;'"; ?>><?php echo $currentGameDescriptionDecrypted; ?></textarea>

		<br />

		<p id="inputError6"><?php echo $nameDoesntExistButImageExistsError; ?></p>

		<label for="gamecoverimage" class="gameCoverImageLabel"><?php echo $currentGameCoverImage; ?></label>

		<input type="file" name="gamecoverimage" id="gamecoverimage" class="gamecoverimage" accept=".png, .jpg, jpeg" />

		<br />

		<input type="submit" id="editButton" name="edit_button" value="Submit updated game for review!" />

		<br />

		<input type="submit" id="deleteButton" name="delete_button" value="Delete game!" />
...
<?php if ($login == 1) : ?>
	...
	<?php
		...
		$query = "SELECT username FROM gamez WHERE gameId = ?;";
		$stmt = $db->prepare($query);
		$stmt->bind_param("s", $id);
		$stmt->execute();
		$stmt->store_result();
		$stmt->bind_result($activeUsername);
		...
		if ($stmt->num_rows > 0) {
			...
			if ($stmt->fetch()) {
				...
				if ($currentUsername == $username) {
					...
					$editGameError = "Error";
					$editGameLoggedInDifferentUser = "You can't edit this game!";
					...
					?>
						...
						<h1 id="game_error_label"><?php echo $editGameError; ?></h1>

						<p id="game_logged_in_different_user"><?php echo $editGameLoggedInDifferentUser; ?></p>
			...
			$editGameError = "Error";
			$gameNotFound = "No game linked with the provided game id in the URL exists!";
			...
			?>
				...
				<p id="game_not_found"><?php echo $gameNotFound; ?></p>
	...
	<div id="back_arrow_div">
		...
		<a href="gamez?username=<?php echo $username; ?>" id="back_arrow_link">
...
<?php if ($login == 0) : ?>
	...
	<?php
		...
		$editGameError = "Error!";
		$editGameNotLoggedIn = "Edit game can't be accessed if not logged in!";
	...
	<p id="game_not_accessible_not_logged_in"><?php echo $editGameNotLoggedIn; ?></p>
...
<script type="text/javascript">
	...
	$("#gamecoverimage").change(function() {

		var file = $("#gamecoverimage")[0].files[0].name;
