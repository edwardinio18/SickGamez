<?php
	...
	if (isset($_POST['create_new_password'])) {
		...
		if (empty($confirmPassword)) {
			...
			$confirmPasswordFieldEmpty = "Confirm password is required!";
			$errors = 1;
		...
		if ($errors == 0) {
			...
			if ($password == $confirmPassword) {
				...
				$sql = "SELECT passwordResetSelector, passwordResetToken, passwordResetEmail FROM passwordReset WHERE passwordResetSelector = ? AND passwordResetToken = ?;";
				$stmt = $db->prepare($sql);
				...
				$stmt->execute();
				$stmt->store_result();
				$stmt->bind_result($passwordResetSelector, $passwordResetToken, $passwordResetEmail);
				$stmt->fetch();

				if ($selector == $passwordResetSelector && $validator == $passwordResetToken) {

					$sql = "SELECT email FROM users WHERE email = ?;";
					$stmt = $db->prepare($sql);
					$tokenEmail = $passwordResetEmail;
					...
					if ($tokenEmail == $email) {
						...
						if (!mysqli_stmt_prepare($stmt, $sql)) {
							...
							if (!mysqli_stmt_prepare($stmt, $sql)) {
								...
								header("location: log_in?password_changed=success");
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Create a new SickGamez password!" />
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<h1 id="forgot_password_label">Create a new password</h1>
	...
	<?php
		...
		if (empty($_GET['selector']) || empty($_GET['validator'])) {
			...
			?>
				...
				<h1 id="password_reset_error"><?php echo $passwordResetError; ?></h1>
			...
			$passwordResetQuery = "SELECT passwordResetSelector, passwordResetToken FROM passwordReset WHERE passwordResetSelector = ? AND passwordResetToken = ?;";
			$stmt = $db->prepare($passwordResetQuery);
			$selector = $_GET["selector"];
			$validator = $_GET["validator"];
			$stmt->bind_param("ss", $selector, $validator);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($passwordResetSelector, $passwordResetToken);
			$stmt->fetch();
			...
			if ($selector == $passwordResetSelector && $validator == $passwordResetToken) {
				...
				?>
					...
					<form action="create_new_password?selector=<?php echo $selector ?>&validator=<?php echo $validator ?>" method="post" style="text-align: center; margin-top: 8%;">
						
						<input type="hidden" name="selector" value="<?php echo $selector ?>;" />

						<input type="hidden" name="validator" value="<?php echo $validator ?>;" />

						<p id="password_field_empty"><?php echo $passwordFieldEmpty; ?></p>

						<p id="passwords_not_matching"><?php echo $passwordsNotMatching; ?></p>

						<input type="password" name="password" placeholder="New password" class="formInputs" id="password" maxlength="30" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

						<br />

						<p id="confirm_password_field_empty"><?php echo $confirmPasswordFieldEmpty; ?></p>

						<input type="password" name="confirmpassword" placeholder="Confirm password" class="formInputs" id="confirmpassword" maxlength="30" <?php if ($errors == 1) echo "style='border: 2.5px solid red;'"; ?> />

						<br />

						<input type="submit" name="create_new_password" id="reset_password" value="Create new password!" />
