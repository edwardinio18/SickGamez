<?php
	...
	if (isset($_GET['followUnfollow']) || isset($_GET['deleteFeedPost']) || isset($_GET['submitComment'])) {
		...
		if ($result->num_rows >= 1) {
			...
			$feedList = "<p id='no_feed_found'>Your feed is currently empty!</p>";
		...
		$query = "SELECT * FROM feed INNER JOIN users ON feed.feedUserId = users.id WHERE feedUserId = ? ORDER BY dateAndTimeFeedSubmitted DESC LIMIT 5;";
		$stmt = $db->prepare($query);
		$stmt->bind_param("i", $userID);
		$stmt->execute();
		$result = $stmt->get_result();
