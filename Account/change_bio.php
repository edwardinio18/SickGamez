...
<?php
	...
	if ($login == 1) {
		...
		if (isset($_POST['bioSubmit'])) {
			...
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($bio, $id);

			if ($stmt->fetch()) {

				$bio = $_POST['bio'];
				$bioTrim = trim($bio);
				$bioStrippedTags = strip_tags($bioTrim);
				$encryptedBio = encrypt($bioStrippedTags, $key);
				...
				if (!mysqli_stmt_prepare($stmt, $sql)) {
					...
					mysqli_stmt_bind_param($stmt, "ss", $encryptedBio, $id);
					mysqli_stmt_execute($stmt);

					mysqli_stmt_close($stmt);
					mysqli_close($db);

					echo "<meta http-equiv='refresh' content='0; url=change_bio?bio_changed=successfully' />";
...
<div id="friends_side_menu" onmouseleave="closeFriendsList()">
	...
	<?php if ($login == 1) : ?>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#change_bio_error").remove();
				$("#change_bio_not_accessible").remove();

			})

		</script>

		<h1 id="change_bio_label">Change my bio</h1>

		<?php

			if (isset($_GET['bio_changed'])) {

				if ($_GET['bio_changed'] == "successfully") {

					$bioChangedSuccessfully = "Your bio has been changed successfully!";

					echo "<meta http-equiv='refresh' content='3; url=change_bio' />";
		...
		<p id="bio_changed_successfully"><?php echo $bioChangedSuccessfully; ?></p>

		<p id="profileBio"><?php echo $decryptedBio; ?></p>

		<form action="" method="post" id="bio_form" style="text-align: center; margin-top: 5%;">
			
			<textarea name="bio" id="bioForm" maxlength="80"></textarea>

			<br />

			<input type="submit" name="bioSubmit" id="bioSubmit" value="Update bio!" />
	...
	<?php if ($login == 0) : ?>
		...
		<h1 id="change_bio_error"><?php $changeBioError = "Error"; echo $changeBioError; ?></h1>

		<p id="change_bio_not_accessible"><?php $changeBioNotAccessible = "You can't change your bio if you're not logged in!"; echo $changeBioNotAccessible; ?></p>
...
<script type="text/javascript">
	...
	setTimeout(function() {
		...
		$("#bio_changed_successfully").fadeOut(300);
