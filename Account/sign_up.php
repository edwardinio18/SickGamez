<?php
	...
	if (isset($_POST["sign_up"])) {
		...
		$strippedTagsUsername = strip_tags($username);
		
		if (empty($strippedTagsUsername)) {
		...
		if ($password != $confirmPassword) {
			...
			$passwordNotMatchingError = "Passwords must match!";
			$passwordError = "";
		...
		if ($row) {
			...
			if ($row["email"] === $email) {
				...
				$emailAlreadyUsedError = "An account with this email already exists!";
				$errors = 1;
		...
		if ($errors == 0) {
			...
			$DBQuerySignUp = "INSERT INTO users (username, email, password, bio, profileImage, lastActivity, coinAmount, backgroundProfileImage, customUsernameTextStyling, notificationsSound, onlineActivityStatus, onlineActivityStatusType, newAboutActive, customActivityStatusTypeActive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
			$stmt = mysqli_stmt_init($db);

			if (!mysqli_stmt_prepare($stmt, $DBQuerySignUp)) {
				...
				$customUsernameTextStyling = "#ffffff";
				$notificationsSound = "";
				...
				mysqli_stmt_bind_param($stmt, "ssssssisssssss", $strippedTagsUsername, $email, $password, $bio, $profileImage, $lastActivity, $coinAmount, $backgroundProfileImage, $customUsernameTextStyling, $notificationsSound, $onlineActivityStatus, $onlineActivityStatusType, $newAboutActive, $customActivityStatusTypeActive);
				mysqli_stmt_execute($stmt);

				mysqli_stmt_close($stmt);
				mysqli_close($db);

				setcookie("username", $strippedTagsUsername, time() + (60 * 60 * 24), '/');
				setcookie("login", 1, time() + (60 * 60 * 24), '/');

				$_COOKIE['username'] = $strippedTagsUsername;
				header("location: profile?username=$strippedTagsUsername");
...
<head>
	...
	<link rel="icon" type="image/jpg" href="favicon.jpg" />
  	...
  	<meta name="description" content="Create your own SickGamez account here!" />
	...
	<style type="text/css">
		...
		#invalidFirstUsernameSpecialCharacterError {
...
<div id="sideNav" onmouseleave="closeNav()">
	...
	<p id="sidenav_name"><a class="sidenav_name_link" href="/index">SickGamez</a></p>
		...
		<ul id="menu_buttons_list">
    		...
    		<li><a class="menu_buttons" id="log_in_link" href="log_in" style="padding: 8px 8px 8px 32px; text-decoration: none; font-size: 27px; display: block; transition: 0.3s; margin-left: -15%;">Log in</a></li>
	...
	<h1 id="sign_up_label">Sign Up</h1>

	<form method="post" action="sign_up" enctype="multipart/form-data" style="text-align: center; margin-top: 3%;">

		<p id="inputError"><?php echo $usernameError ?></p>

		<p id="inputError"><?php echo $usernameTakenError ?></p>
		...
		<p id="inputError"><?php echo $emailError ?></p>

		<p id="emailAlreadyUsedError"><?php echo $emailAlreadyUsedError ?></p>
		...
		<p id="inputErrorPassword"><?php echo $passwordNotMatchingError ?></p>
		...
		<p id="inputError"><?php echo $confirmPasswordError ?></p>
		...
		<input type="submit" name="sign_up" id="submit" value="Create my account!" />
