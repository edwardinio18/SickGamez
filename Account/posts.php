<?php
	...
	$profileNotAccessible = "Posts can't be accessed!";
	$profileAndUserError = "Error";
	$postID = "";
	$postEditedSuccessfully = "";
	$postDeletedSuccessfully = "";
	$postImageDeletedSuccessfully = "";
...
<head>
	...
	<style type="text/css">
	    ...
	    .profile_pic_display_posts_for_others {
	    ...
	    .post_text_date_time_for_others {
	    ...
	    .zoomed_in_post_image_cover_div {
	    ...
	    .close_zoomed_in_post_image_cover_div_modal {
	    ...
	    .close_zoomed_in_post_image_cover_div_modal:hover, .close_zoomed_in_post_image_cover_div_modal:focus {
	    ...
	    #post_image_deleted_successfully {
					...
					?>
						...
						<script type="text/javascript">
							...
							$(document).ready(function() {
								...
								$("#posts_label_for_others").remove();
								$("#posts_label_for_others_2").remove();
								$("#number_of_posts_for_others").remove();
								$("#no_posts_found_for_others").remove();
								$(".posts_div_for_others").remove();
			...
			while ($row = $selectPostsQueryResult->fetch_assoc()) {
				...
				if ($usernameDB === $currentUsername) {
					...
					if ($myPostImage !== "") {
						...
						$myPostZoomedInCoverDiv = "zoomed_in_post_image_cover_div_" . $myPostID;
					...
					if ($myPosts === "") {
						...
						$myPostsList .= "<div class='posts_div'>
											...
											<img src='" . $myPostImagePath . "' class='post_image' id='post_image_" . $myPostID . "' data-postimageid='" . $myPostID . "' />

											<form action='edit_post?id=" . $postID . "' method='post'>
														
												<input type='submit' name='edit_posts_button' id='edit_posts_button' value='Edit' />
...
<?php
	...
	if (isset($_GET['post_edited'])) {

		if ($_GET['post_edited'] == "success") {

			$postEditedSuccessfully = "Your post has been successfully edited!";

			$username = $_COOKIE['username'];

			echo "<meta http-equiv='refresh' content='3; url=posts?username=$username' />";
	...
	if (isset($_GET['post_deleted'])) {
		...
		if ($_GET['post_deleted'] == "success") {

			$postDeletedSuccessfully = "Your post has been successfully deleted!";
	...
	if (isset($_GET['post_image_deleted'])) {

		if ($_GET['post_image_deleted'] == "success") {

			$postImageDeletedSuccessfully = "Your post image has been successfully deleted!";
...
<h1 id="posts_label">My Posts</h1>

<p id="post_edited_successfully"><?php echo $postEditedSuccessfully; ?></p>

<p id="post_deleted_successfully"><?php echo $postDeletedSuccessfully; ?></p>

<p id="post_image_deleted_successfully"><?php echo $postImageDeletedSuccessfully; ?></p>
...
<?php if ($login == 1 && $selectPostsQueryResult->num_rows > 0) : ?>
		...
		if ($selectPostsQueryResult->num_rows >= 1) {
			...
			while ($row = $selectPostsQueryResult->fetch_assoc()) {
				...
				if ($usernameDB === $currentUsername) {
					...
					$strippedTagsPostForOthers = strip_tags($decryptedPostForOthers);
					$dateAndTimePostAdded = $row['dateAndTimeAdded'];
					$formattedDateAndTimePostAdded = date("H:i, d/m/Y", strtotime($dateAndTimePostAdded));
					...
					$myPostIDDBForOthers = "post_text_div_for_others_" . $myPostIDForOthers;
					$myPostDivIDDBForOthers = "posts_div_" . $myPostIDForOthers;

					$myPostZoomedInCoverDivForOthers = "zoomed_in_post_image_cover_div_" . $myPostIDForOthers;
					...
					if ($myPostsForOthers === "") {
						...
						$myPostsListForOthers .= "<div class='posts_div_for_others'>
				
													<img src='" . $userProfilePicturePostsPath . "' class='profile_pic_display_posts_for_others' />

													<p class='post_text_date_time_for_others'> " . $formattedDateAndTimePostAdded . " </p>

													<img src='" . $myPostImagePathForOthers . "' class='post_image' id='post_image_" . $myPostIDForOthers . "' data-postimageid='" . $myPostIDForOthers . "' />
						...
						if ($myPostImageForOthers === "") {
							...
							$myPostsListForOthers .= "<div class='posts_div_for_others'>
														...
														<div class='post_text_div_for_others' id='post_text_div_for_others_" . $myPostIDForOthers . "'>
															
															<p class='main_post_text_for_others'> " . $strippedTagsPostForOthers . " </p>
	...
	<div id="posts_label_for_others_div">
		...
		<h1 id="posts_label_for_others"><?php echo $username; ?></h1><h1 id="posts_label_for_others_2">'s Posts</h1>

	</div>

	<p id="number_of_posts_for_others"><?php echo $postsCounter; ?></p>
...
<?php if ($login == 0 && $selectPostsQueryResult->num_rows > 0) : ?>
		...
		if (isset($_GET['username'])) {
			...
			if ($queryResult->num_rows >= 1) {
				...
				if (isset($_GET['username'])) {
					...
					$username = $_GET['username'];
					$noPosts = $username . " currently has no posts!";
					$postsCounter = "0 posts";
					...
					?>
						...
						<p id="no_posts_found_for_others"><?php echo $noPosts; ?></p>
...
<?php if ($login == 1 && $selectPostsQueryResult->num_rows == 0) : ?>
	...
	<?php
		...
		if (isset($_GET['username']) && $queryResult->num_rows > 0 && $_GET['username'] == $username) {
...
<script type="text/javascript">
	...
	setTimeout(function() {
		...
		$("#post_edited_successfully").fadeOut(300);

	}, 2000);

	setTimeout(function() {

		$("#post_deleted_successfully").fadeOut(300);
	...
	$(document).on("click", ".post_image", function() {
		...
		var postImageId = $(this).data("postimageid");
		var postImage = $(this).attr("id");

		$("#post_image_" + postImageId).css("position", "fixed");
		$("#post_image_" + postImageId).css("max-width", "90%");
		$("#post_image_" + postImageId).css("max-height", "90%");
		$("#post_image_" + postImageId).css("height", "auto");
		$("#post_image_" + postImageId).css("width", "initial");
		$("#post_image_" + postImageId).css("z-index", "10");
		$("#post_image_" + postImageId).css("left", "0");
		$("#post_image_" + postImageId).css("right", "0");
		$("#post_image_" + postImageId).css("top", "0");
		$("#post_image_" + postImageId).css("bottom", "0");
		$("#post_image_" + postImageId).css("margin", "auto");
		$("#post_image_" + postImageId).css("transition", "0.15s");
		$("#post_image_" + postImageId).css("border-radius", "0.75vw");
		$("#post_image_" + postImageId).css("pointer-events", "none");
		$("body").css("overflow", "hidden");

		$("body").append("<div class='zoomed_in_post_image_cover_div' id='zoomed_in_post_image_cover_div_" + postImageId + "' data-zoomedpostimage='" + postImageId + "'><span class='close_zoomed_in_post_image_cover_div_modal' id='close_zoomed_in_post_image_cover_div_modal_" + postImageId + "' data-postimageclosemodalspan='" + postImageId + "'>&times;</span></div>");
		$(".zoomed_in_post_image_cover_div").css("display", "block");
		$("#post_image_" + postImageId).addClass("zoomed_in_post_image");

	});

	$(document).on("click", ".close_zoomed_in_post_image_cover_div_modal", function() {

		var closePostImageModalSpan = $(this).data("postimageclosemodalspan");

		$(".zoomed_in_post_image_cover_div").remove();
		$("#post_image_" + closePostImageModalSpan).removeAttr("style");
		$("#post_image_" + closePostImageModalSpan).removeClass("zoomed_in_post_image");
	...
	$("html").bind("keydown", function(e) {
		...
		if (e.keyCode === 27) {
			...
			if ($(".zoomed_in_post_image_cover_div").is(":visible")) {

				var zoomedPostImageId = $(".zoomed_in_post_image_cover_div").data("zoomedpostimage");

				$(".zoomed_in_post_image_cover_div").remove();
				$("#post_image_" + zoomedPostImageId).removeAttr("style");
				$("#post_image_" + zoomedPostImageId).removeClass("zoomed_in_post_image");
				$("body").css("overflow", "auto");
