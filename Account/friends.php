<?php
	...
	$profileNotAccessible = "Friends can't be accessed!";
	$profileAndUserError = "Error";

	$myFriendsList = "";
	$friendsCounter = 0;
	$myFriendsListForOthers = "";
	$friendsListEmptyForOthers = "";
	$friendsListEmpty = "";
...
<head>
	...
	<style type="text/css">
	    ...
	    .profile_pic_display_friends_for_others {
	    ...
	    .friend_username_div_for_others {
	    ...
	    .friend_user_link_for_others:hover {
					...
					?>
						...
						<script type="text/javascript">
							...
							$(document).ready(function() {
								...
								$("#friends_label_for_others").remove();
								$("#friends_label_for_others_2").remove();
								$("#number_of_friends_for_others").remove();
								$("#no_friends_found_for_others").remove();
								$(".friends_div_for_others").remove();
...
<?php
		...
		if ($result->num_rows >= 1) {
			...
			if ($numberOfFriendsQueryResult->num_rows >= 1) {
				...
				while ($row = $numberOfFriendsQueryResult->fetch_assoc()) {
					...
					$selectFriendDataQuery = "SELECT * FROM friends INNER JOIN users ON friends.friend2Id = users.id ORDER BY dateAndTimeBecameFriends DESC;";
					...
					if ($selectFriendDataQueryResult->num_rows >= 1) {
						...
						while ($row = $selectFriendDataQueryResult->fetch_assoc()) {
							...
							if ($myID === $row['friend1Id']) {
								...
								$friendsSinceDate = $row['dateAndTimeBecameFriends'];
								$formattedFriendsSinceDate = date("d/m/Y", strtotime($friendsSinceDate));
								$friendsSinceStatement = "Friends since " . $formattedFriendsSinceDate;
								...
								$friendUsernameIdDB = "friend_username_" . $friendUsernameId;
								...
								$myFriendsList .= "<div class='friends_div'>
														...
														<p class='friends_since'>" . $friendsSinceStatement . "</p>

														<div class='friend_username_div'>
															
															<a href='profile?username=" . $friendUsername . "' class='friend_user_link'>

																<p class='friend_username' id='friend_username_" . $friendUsernameId . "'>" . $friendUsername . "</p>
														...
														<form action='' method='post'>
															...
															<input type='hidden' name='id' value='" . $friendUsernameId . "' />
													
															<input type='submit' name='remove_friend' id='remove_friend' value='Remove friend!' />
	...
	if (isset($_POST['remove_friend'])) {
		...
		if ($result->num_rows >= 1) {
			...
			$selectRemoveFriendQuery = "SELECT * FROM friends WHERE friend1Id = ? AND friend2Id = ?;";
			$stmt = $db->prepare($selectRemoveFriendQuery);
			$stmt->bind_param("ii", $myID, $userID);
			$stmt->execute();
			$selectRemoveFriendQueryResult = $stmt->get_result();

			if ($selectRemoveFriendQueryResult->num_rows >= 1) {

				if ($row = $selectRemoveFriendQueryResult->fetch_assoc()) {

					$query = "DELETE FROM friends WHERE friend1Id = ? AND friend2Id = ?;";
						...
						if ($selectRemoveFriendQueryResult->num_rows >= 1) {
							...
							if ($row = $selectRemoveFriendQueryResult->fetch_assoc()) {
								...
								if (!mysqli_stmt_prepare($stmt, $query)) {
									...
									echo "<meta http-equiv='refresh' content='0; url=friends?username=$username' />";
...
<?php if ($login == 1 && $numberOfFriendsQueryResult->num_rows >= 1) : ?>
	...
	<?php
					...
					if ($selectFriendDataQueryResult->num_rows >= 1) {
						...
						while ($row = $selectFriendDataQueryResult->fetch_assoc()) {
							...
							if ($userID === $row['friend1Id']) {
								...
								$friendUsernameIdDB = "friend_username_for_others_" . $friendUsernameId;
								...
								$myFriendsListForOthers .= "<div class='friends_div_for_others'>
			
														<img src='" . $userProfilePictureFriendPath . "' class='profile_pic_display_friends_for_others' />

														<div class='friend_username_div_for_others'>
															
															<a href='profile?username=" . $friendUsername . "' class='friend_user_link_for_others'>

																<p class='friend_username_for_others' id='friend_username_for_others_" . $friendUsernameId . "'>" . $friendUsername . "</p>
		...
		if ($result->num_rows >= 1) {
			...
			if ($numberOfFriendsQueryResult->num_rows >= 1) {
				...
				$friendsCounter = "0 friends";
				$errors = 1;
				$friendsListEmptyForOthers = $username . "'s friends list is currently empty!";
	...
	<div id="friends_label_for_others_div">
		...
		<h1 id="friends_label_for_others"><?php echo $username; ?></h1><h1 id="friends_label_for_others_2">'s Friends</h1>

	</div>

	<p id="number_of_friends_for_others"><?php echo $friendsCounter; ?></p>

	<p id="no_friends_found_for_others"><?php echo $friendsListEmptyForOthers; ?></p>
...
<?php if ($login == 0 && $numberOfFriendsQueryResult->num_rows >= 1) : ?>
...
<?php if ($login == 0 && $numberOfFriendsQueryResult->num_rows == 0) : ?>
...
<?php if ($login == 1 && $numberOfFriendsQueryResult->num_rows == 0) : ?>
