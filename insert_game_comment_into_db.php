<?php
	...
	if (isset($_GET['gameCommentContent'])) {

		$gameCommentContent = $_GET['gameCommentContent'];

		$trimmedGameCommentContent = trim($gameCommentContent);
		$strippedTagsGameComment = strip_tags($trimmedGameCommentContent);
		...
		if ($strippedTagsGameComment === "") {
			...
			$insertGameCommentQuery = "INSERT INTO gameComments (gameCommentGameId, gameCommentUserId, gameCommentContent) VALUES (?, ?, ?);";
			$gameCommentStmt = $db->prepare($insertGameCommentQuery);
			$encryptedGameComment = encrypt($strippedTagsGameComment, $key);
			$gameCommentStmt->bind_param("iis", $gameIdFromDB, $myID, $encryptedGameComment);
